...
ZackAI::ZackAI(int num_x, int num_y) : num_cells_x(num_x), num_cells_y(num_y) {
...
Uint8 &ZackAI::lookup_at(Coord c) {
...
Direction ZackAI::move(Coord game_fruit, Direction last_move,
                       const std::deque<Coord> &game_snake, int snake_growing) {
  ...
  if (chosen_direction != Direction::NONE) {
    ...
    // TODO: This currently space fills only from top to bottom, make it
    // able
